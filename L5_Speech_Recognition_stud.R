...
# LESSON 5: Speech Recognition Lab Exercise
...
# Experiment 6: Shiny Dashboard
install.packages("shiny")

library(shiny)
library(googleLanguageR)

# --- 1. USER INTERFACE (The 'Front-End') ---
ui <- fluidPage(
  theme = bslib::bs_theme(bootswatch = "flatly"), # Optional: makes it look professional
  titlePanel("Voice-Controlled MIS Dashboard"),
  
  sidebarLayout(
    sidebarPanel(
      helpText("Click the button to simulate a voice command."),
      actionButton("record_btn", "Analyze Voice Command", class = "btn-primary", icon = icon("microphone"))
    ),
    
    mainPanel(
      h4("Transcription Result:"),
      verbatimTextOutput("command_text"),
      hr(),
      h4("Data Output:"),
...
# --- 2. SERVER LOGIC (The 'Back-End') ---
server <- function(input, output, session) {
  
  # This triggers ONLY when the button is clicked
  voice_data <- eventReactive(input$record_btn, {
    # Show a loading message for the students
    showModal(modalDialog("AI is processing audio...", footer = NULL))
    
    # Using your stable demo file instead of live recording
    # In a real app, this would be the 'record' logic
    result <- gl_speech("demo_audio.wav", languageCode = "en-US")
    
    removeModal()
    
    # Return the text and the data
    return(result$transcript$transcript[1])
  })
  
  # Update the text display
  output$command_text <- renderText({
    req(voice_data()) # Wait until the button is clicked
    paste("System heard:", voice_data())
  })
  
  # Update the table based on the 'voice'
  output$data_output <- renderTable({
    req(voice_data())
    
    # Simple logic: If the voice mentions "data", show mtcars
    # Otherwise, show a small sample
    if(grepl("data", tolower(voice_data()))) {
      return(head(mtcars, 5))
    } else {
      return(head(iris, 3))
    }
  })
