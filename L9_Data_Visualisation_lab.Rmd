...
# Loading the dataset from the subfolder
covid_df <- read_csv("Covid 19 Dataset/covid_19_clean_complete.csv")
```
...
# 1. We use backticks `` because of the slash in the column name
# 2. We filter for is.na() or empty strings if "All States" isn't present
covid_df_all_states <- covid_df %>% 
  filter(is.na(`Province/State`) | `Province/State` == "") %>% 
...
covid_df_all_states_daily <- covid_df_all_states %>% 
  select(Date, `Country/Region`, Active, Confirmed, Deaths, Recovered)
...
```{r}
# 1. Rename columns to match your lab's expected variable names
covid_cleaned <- covid_df_all_states %>%
  rename(
    Country_Region = `Country/Region`,
    active = Active,
    positive = Confirmed  # In this dataset, Confirmed is the total positive count
  )

# 2. Summarise based on available columns
# Note: daily_tested and hospitalizedCurr are missing from this specific CSV
covid_df_all_states_daily_sum <- covid_cleaned %>% 
...
## Getting vectors
```{r}
# Chunk 1: Extract vectors 
countries <- covid_top_10$Country_Region
positive_cases <- covid_top_10$total_positive
active_cases <- covid_top_10$total_active

# Creating placeholders for missing columns so the rest of the lab works
tested_cases <- rep(NA, length(countries))
hospitalized_cases <- rep(NA, length(countries))
```

## Naming vectors
```{r}
# Chunk 2: Assign the country names to each vector
names(positive_cases) <- countries
names(tested_cases) <- countries
names(active_cases) <- countries
names(hospitalized_cases) <- countries

# Quick check to see the named vector for Positive Cases
positive_cases
```
